...
def get_systeminfo(ipaddress, config, interactive=False):
    ...
    if (ipaddress in utils.get_local_ips() or
            ipaddress_module.ip_address(utils.unicode(ipaddress)).is_loopback):
...
def system_info(client):
    ...
    if client.is_windows():
        ...
        output = client.execute(command, escalate=True)
        ...
        if any(m in k for m in not_found_msgs
            ...
            raise errors.SystemInfoCommandMissing("ohai-solo missing on %s" %
                                                  client.host)
        unicode_output = utils.unicode(output['stdout'], errors='replace')
        try:
...
def install_remote(client):
    ...
    if is_windows:
        ...
        ohai_cmd = "wget -N http://ohai.rax.io/install.sh"
        install_ohai_cmd = "bash install.sh"
        rm_install_cmd = "rm install.sh"

        # Download to host
        command = ohai_cmd
        client.execute(command, wd='/tmp', escalate=True)
    ...
    if client.is_windows():
        ...
        if client.is_debian():
            ...
            raise errors.UnsupportedPlatform("Unknown distro: %s" %
                                             platform_info['dist'])
        command = "%s" % remove
        output = client.execute(command, wd='/tmp', escalate=True)
        return output
